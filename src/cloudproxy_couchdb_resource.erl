...
-module(cloudproxy_couchdb_resource).
-export([init/1,
         service_available/2]).
...
service_available(RP, C={_ExternalPath, CouchPath}) ->
    ...
    Headers = clean_request_headers(mochiweb_headers:to_list(wrq:req_headers(RP))),
    Method = wm_to_ibrowse_method(wrq:method(RP)),
    ReqBody = case wrq:req_body(RP) of
                  undefined -> [];
                  B -> B
              end,
	error_logger:info_report(Path),
	error_logger:info_report(Headers),
	error_logger:info_report(Method),
